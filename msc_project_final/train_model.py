...
# Automatically finds the project root folder
...
def add_engineered_features(df):
    """Creates consistent features used by training and prediction."""
    df = df.copy()
...
def main():
    ...
    # Prepare X and y
    exclude_cols = [TARGET_COLUMN, "date", "route"] 
    ...
    anomaly_df = df[existing_anom_feats].copy().apply(pd.to_numeric, errors='coerce').fillna(0)
    ...
    print(f"✅ Training Success! Accuracy: {metrics_data['accuracy']:.4f}")
    print(f"Files saved successfully in: {MODEL_DIR}")
